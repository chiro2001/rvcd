...
fn with_event_loop<R>(
    ...
    EVENT_LOOP.with(|event_loop| {
        ...
        let event_loop = if let Some(event_loop) = &mut *event_loop_lock {
            log::warn!("EVENT_LOOP.with Some");
            event_loop
        } else {
            log::warn!("EVENT_LOOP.with None");
            event_loop_lock.insert(create_event_loop(&mut native_options)?)
        };
        log::warn!("EVENT_LOOP.with after insert");
...
pub fn run_glow(
    ...
    if native_options.run_and_return {
        log::warn!("Glow with_event_loop");
        return with_event_loop(native_options, |event_loop, native_options| {
            log::warn!("Glow with_event_loop with GlowWinitApp");
            let glow_eframe = GlowWinitApp::new(event_loop, app_name, native_options, app_creator);
            log::warn!("Glow run_and_return");
